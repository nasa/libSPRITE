...
namespace SCALE
{
    class Task_properties
    {
        public:
            ...
            {
                luaW_register<SRTX::Task_properties>(L, "Task_properties",
                        NULL, methods);
                return 0;
            }
            ...
            static int init(lua_State *L)
            {
                ...
                {
                    case 4:
                        p->stack_size = static_cast<unsigned int>(
                                luaL_checknumber(L, 5));
                        // Intentional fallthrough
                    case 3:
                        p->schedule_presence = static_cast<
                            SRTX::schedule_presence_t>(luaL_checknumber(L, 4));
                        // Intentional fallthrough
                    case 2:
                        p->period = units::Nanoseconds(
                                units::Seconds(luaL_checknumber(L, 3)));
                        // Intentional fallthrough
                    case 1:
                        p->prio = static_cast<SRTX::priority_t>(
            ...
            {
                SRTX::Task_properties* p = luaW_check<SRTX::Task_properties>(L, 1);
                p->period = units::Nanoseconds(units::Seconds(
                            luaL_checknumber(L, 2)));
                return 0;
            }
